...
export interface IPhoneDetails {
    ...
    review_url?: string;
    /** Slugs of related device variants found on the specs page (e.g. _5g variants) */
    siblingDeviceSlugs: string[];
    /** Full data from the pictures page — press renders, color variants, page URL */
...
export interface IPhoneListItem {
    ...
    slug: string;
    imageUrl?: string;
    thumbUrl?: string;   // Small thumbnail src from GSMArena list pages
    detail_url: string;
    rank?: number;
    hits?: number;
}
